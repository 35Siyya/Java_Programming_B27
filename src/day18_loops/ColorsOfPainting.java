...
public class ColorsOfPainting {
    ...
    public static void main(String[] args) {
        ...
        int counter = 1;
        while (counter++ <= 3) { //1 red,2 red,3 cycle

            System.out.print("Enter a color: ");
            String colorEntered = input.nextLine(); //user entered RED ,

            if (storedColors.contains(colorEntered)) { //red == red -> change the colour
                System.out.println("Please enter a unique color.");
                counter--; //2-1=1, one more chance to enter color -> counter = counter -1;

            } else { //if all unique colours: red,blue,green
